...
\begin{figure}[]
\centering
\includegraphics[width=0.95\linewidth]{caloFits_chi2_fourDatsets}
...
\vspace{4mm}
\includegraphics[width=0.95\linewidth]{caloFits_R_fourDatsets}
    \caption[$R$ versus calorimeter number]{$R$ versus calorimter for the Run~1 precession frequency analysis datasets. A straight line fit was performed on the fitted values, with the fit result shown in the upper right box as parameter $p_{1}$ in units of ppm. The scale is the same on each of the plots, the higher precision Endgame dataset is reflected in the smaller error bars.}
...
-describe here a little better how useful a tool the fit start scans are - sort of get into it in the text but emphasis the tool itself
...
\centering
    \begin{subfigure}[]{0.45\textwidth}
        ...
        \caption{p value of first ratio fit in fit start scan without changing cob frequency - 0.191}
    \end{subfigure}% %you need this % here to add spacing between subfigures
    \vspace{1cm}
    \begin{subfigure}[]{0.45\textwidth}
        \centering
...
\begin{figure}[]
    ...
    \caption[\chisq/NDF and $R$ versus fit start time]{\chisq/NDF versus fit start time for the Run~1 precession frequency analysis datasets. The fit points lie in and around the $1\sigma$ statistical bands. $R$ versus fit start time for the Run~1 precession frequency analysis datasets. The fit points lie in and around the $1\sigma$ statistical bands.}
    \label{fig:fig:fitStartTime_chi2_and_R}
\end{figure}
\end{landscape}
