...
The STDP correction is in many ways the same as the IFG correction procedure \cite{STDP}. This correction is applied to pulses close in time, $\mathcal{O}(\text{ns})$, and is the first gain correction applied to the energies of the pulses before the out-of-fill and in-fill gain corrections. For the systematic error on $R$ due to the application of the STDP gain correction, a new production of the 60h dataset was processed without the STDP correction applied. Because of how the time-randomization is applied to the clusters, a new version of the same dataset by default uses a different time-randomization. As shown in \secref{sub:randomSeedFits}, the width in the fitted $R$ values is of order $\mathcal{O}(\SI{100}{ppb})$ making direct ratio fit comparisons between the two datasets un-informative. In order to avoid this difficulty, fits were done with the T-Method and cluster times were randomized per-fill rather than per-cluster. Since the per-fill randomization uses fill ID's as the seeds for the randomization, and because the T-Method does not split the data into sub-datasets as the Ratio Method does, the randomization between the 60h dataset with and without the STDP correction is identical. The change in $R$ for T-Method fits with and without this correction is then taken as the upper bound on the systematic error for the inclusion of the STDP in the Ratio Method fits. This is reasonable as the previous IFG systematic studies showed a reduction in sensitivity to gain effects which is probably extended to the STDP correction error. \tabref{tab:systematicError_STDP} gives the T-Method fit results with and without the STDP, along with the change in $R$ which came out to be \SI{11.0}{ppb}. This is taken as the systematic error for all datasets.
...
\tabref{tab:GainErrorsTotal} gives the quadrature sum for the total gain systematic errors for the Run~1 precession frequency analysis datasets. As shown for each dataset the total error is below the target final error of \SI{20}{ppb}. It should be noted that there is evidence of the need for a correction for some type of residual gain effect \cite{AFThesis,SweigartEndgameOddities} which is somehow unmeasurable by the laser system. This combined with recently discovered small errors in the gain corrections applied to the Endgame and HighKick datasets \cite{GainOscEndgameHighKick} imply that these errors will change by some amount. Because the Ratio Method sensitivities are so much more reduced to the gain corrections than the T-Method however, the numbers given here are a reasonable estimation of where the final errors will lie. In future runs of E989 with more statistics it will be important to verify that hidden small effects do not balloon and negatively impact the \wa extraction.
...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lGGGG}
  \hline
    \multicolumn{5}{c}{\textbf{Total Pileup Systematic Errors}} \\
  \hline\hline
