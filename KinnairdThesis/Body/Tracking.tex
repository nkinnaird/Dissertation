...
In order to fit the track, the $\chi^{2}$ is minimized with respect to the initial track parameters $p$, and evaluated at a specific set of those starting paramters $p'_{0}$:
    ...
    \end{aligned}
    \end{equation}
The middle term is small assuming that that the error matrix doesn't change much with respect to the choice of starting parameters. This is true as the error matrix is small due to the low amount of material in the tracker. (In tandem the error matrix doesn't change much from one fitting iteration to the next as long as the path length through the material remains about the same.) The first and third terms are identical in value, and so must therefore both separately be equal to zero. \equref{eq:minimize} is therefore reduced to 
        \begin{align} \label{eq:solve}
            0 = \sum_{i=1}^{N} T^{T}_{i0} (\sigma_{i}^{-1}) (p_{i}(p'_{0})-x_{i}),
        \end{align}
where $T_{i0}$ is the transport matrix between the starting parameters and plane $i$
