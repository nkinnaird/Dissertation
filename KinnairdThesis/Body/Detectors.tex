...
Second, they must have a timing resolution of better than $\SI{100}{ps}$ for positrons with greater than $\SI{100}{\MeV}$ \cite{TDR}. The calorimeters must be able to resolve multiple incoming hits through temporal or spatial separation at 100\% efficiency for time separations of greater than 5 ns in order to reduce the pileup systematic effect due to the high rate. Pileup is the term used for when multiple hits impact the detector within the deadtime such that they are registered as a single hit. Unresolved pileup means that the number of detected positrons above threshold is mis-measured. Since pileup is a time-dependent effect, where pileup decays at some lifetime approximately equal to half the muon lifetime, this leads to a mis-measurement of \wa. The requirements stated here assist in reducing the pileup systematic error below the target goal of $\SI{40}{ppb}$.

Third, the gain of the measured hits must be stable to $<$ 0.1\% over a \mus{200} time period within a fill (\mus{700}), and unaffected by a pulse arriving in the same channel a few nanoseconds earlier \cite{TDR}. The long term gain stability over a time period of order seconds must be $<$ 1\%. Gain is the term used for the energy response of the detector as a function of time. After a hit, the measured energy fraction of a following hit drops sharply and then rise exponentially back to one. Hits with mis-measured energies can thus be excluded if their energies are below threshold, leading to another systematic effect and subsequent error in the \wa measurement. The requirements stated here, along with the use of a laser calibration system (\secref{sub:LaserCalibrationSystem}), assist in reducing the gain systematic error below the target goal of $\SI{20}{ppb}$.


\subsection{Harware}


There are 24 calorimeters located symmetrically around the inside of the ring placed flush to the vacuum chamber wall, as shown in \figref{fig:CalorimeterPositions}. (Indeed the vacuum chamber was designed so as to reduce multiple scattering of the decay positrons before entering the calorimeters.) Each calorimeter consists of 54 channels of PbF\textsubscript{2} crystals arrayed in a 6 high by 9 wide array. Each crystal is 2.5 x 2.5 x 14 cm\textsuperscript{3} and is wrapped in black Tedlar\textregistered\ foil. The PbF\textsubscript{2} material has an index of refraction of 1.8, and emits Cerenkov light as incident positrons pass through the crystals \cite{Fienberg:2014kka}. Cerenkov light is naturally fast which improves the timing resolution of the incoming hits. It's high density gives it a short radiation length, such that the energy deposition from the incident positrons is nearly 100\% over the length of the crystal. The black foil is used to reduce light transmission between crystals and improve position reconstruction, as well as reduce pulse width \cite{Kaspar:2016ofv}. The energy deposition from the incident positrons is typically restricted to only one or two crystals. The segmentation of the calorimeter combined with the black wrapping helps the spatial and temporal resolution of the detected pulses.
...
\centering
    ...
    \end{subfigure}
\caption[Calorimeters]{\bf{probably update these pictures at some point}}
\label{fig:calos}
\end{figure}



Each crystal is paired with a large area silicon photo-multiplier (SiPM) sensor which reads out the emitted Cerenkov light. SiPMs are compact, operable in high magnetic field regions, have very linear responses for operation at MHz rates, are suited to measuring Cerenkov light due to their high photo-detection efficiency in the wavelengths of interest, and a high degree of gain stability \cite{Kaspar:2016ofv}. The SiPMs used are designed by Hamamatsu and detailed in \refref{Kaspar:2016ofv}. They sit on printed circuitry boards (PCBs) designed to preserve the fast SiPM pulse shape. The combined properties of the chosen SiPMs, their electronic boards, and the PbF\textsubscript{2} crystals results in an energy resolution of $(4.6 \pm 0.3) \% / \sqrt{E/\GeV}$, a timing resolution of $\SI{20}{ps}$, and a pulse width of $\SI{5}{ns}$, satisfying the stated requirements earlier \cite{Fienberg:2014kka,Kaspar:2016ofv}. Details of the pulse fitting algorithms are shared in \secref{sec:ReconWest}. See \figref{fig:CalorimeterCrystals} for a picture of the calorimeter crystals and associated SiPMs.



- should I get a picture of what the energy deposition looks like? or a sipm pulse? etc 




Each calorimeter sits on a board extending out from a cart containing the electronic which power the calorimeters and read out the data, as shown in Figure \ref{fig:CalorimeterPositions} This is to relocate magnetic material away from the field region to avoid perturbing the field and to remove sensitive electronics from the decay path region. Similarly, due to the close proximity to the storage region and by extension the magnetic field, the calorimeters, the encapsulating material, and the SiPMs are made from non-magnetic material.
...
-have I mentioned non-magnetic side of things


% \begin{figure}[]
%     \centering
%     \includegraphics[width=0.5\textwidth]{CalorimeterPositions}
%     \caption[Model of calorimeter positions relative to vacuum chamber]{}   
%     \label{fig:CalorimeterPositions}
% \end{figure}
