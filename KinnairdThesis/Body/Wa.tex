...
The parameter $\tau$ is the same muon lifetime as in the T-Method fit function, except here it is set as the default value of \mus{64.44}\footnote{The fitted \wa frequncy is largely insensitive to this parameter, and especially so in the Ratio Method fits, so this is acceptable.}. The value for $t_{0}$ can be taken as any time at or before the start of the fitting range, as it simply changes the scale of $N_{0}$ which is largely unimportant. The modification $\Lambda(t)$ as listed in \equref{eq:TmethodFunction} is then taken as
    ...
    \end{align}
where $\boldsymbol{\kappa_{loss}} = 1/\epsilon N_{0}$ is taken as the fit parameter\footnote{By construction $\kappa_{loss}$ is a very small parameter, $\mathcal{O}(10^{-10})$. This factor is absorbed into the parameter in such that the fit parameter is $\mathcal{O}(1)$.}. The fractional losses (the integral in \equref{eq:lambdalosses} times the final fitted $\kappa_{loss}$ parameter) for the Run~1 precession frequency analysis datasets are shown in \figref{fig:fractionallosses}.
...
\centering
    ...
    \begin{subfigure}[]{0.45\textwidth}
        ...
        \caption{Sensitivity of $R$ to value of SDT. Units are in ppm.}
    \end{subfigure}
    ...
    \end{subfigure}
\caption[Pileup shadow dead time scan without automatic pileup amplitude scaling]{Shadow dead time scan results without automatic pileup amplitude scaling. A clear minimum in the \chisq plot is seen near \ns{5} corresponding to the choice of ADT, and a steep slope in the slope for $R$ is observed. In the bottom two spectra plots the magenta curve corresponds to a choice SDT = \ns{5} while the black curve corresponds to SDT = \ns{10}. The larger choice of SDT leads to an greater estimation of the pileup, which in the energy spectra plot leads to a corresponding over-subtraction in the energy spectrum at energies where hits consist mostly or purely of pileup pulses. Data from 9d dataset.}
...
\centering
    ...
    \begin{subfigure}[]{0.45\textwidth}
        ...
        \caption{The corrected energy spectrum for different choices of SDT.}
    \end{subfigure}
\caption[Pileup shadow dead time scan with automatic pileup amplitude scaling]{Shadow dead time scan results with automatic pileup amplitude scaling. No clear minimum is observed in the \chisq plot, and the slope for $R$ is small. In the bottom two spectra plots the magenta curve corresponds to a choice SDT = \ns{5} while the black curve corresponds to SDT = \ns{10}. With the automatic amplitude scaling applied, the time and energy spectra lie on top of each other. That combined with the lack of clear minimum in the \chisq plot and small slope in $R$ indicate that there is no real systematic error due to the choice of SDT. Data from 9d dataset.}
\label{fig:SDTscan_autoScaling}
\end{figure}


Similarly to the scan over SDT, scanning over the choice of SGT (default value of \ns{10} or twice the SDT) allows for determination of a potential systematic error. The results of such a scan with the automatic pileup amplitude scaling is shown in \figref{fig:SGTscan}. Just as in the SDT scan with the automatic scaling, there is no minimum in the \chisq plot, the slope in $R$ is negligible, and the pileup spectra for the various choices of SGT are nearly identical. Therefore again any systematic error due to the choice of SGT is subsumbed into the pileup amplitude error.
...
\centering
    ...
    \begin{subfigure}[]{0.45\textwidth}
        ...
        \caption{Sensitivity of $R$ to value of SGT. Units are in ppm.}
    \end{subfigure}
...
\centering
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        ...
        \caption{Sensitivity of $R$ to value of pileup multiplier. Units are in ppm.}
    \end{subfigure}
...
The pileup phase error is the error on $R$ assuming the phase of the pileup was incorrectly constructed. This is separated into two parts. The first part is calculated by applying time-shifts to \equref{eq:tdoublet}, assuming that the time of the constructed shadow doublet was incorrect in some way. A scan over pileup time-shifts is shown in \figref{fig:PTSscan}. A clear slope in $R$ is observed, however there is no clear minimum in the \chisq plot. Because the width of the \chisq curve cannot be taken as the uncertainty in the pileup time-shift, instead the uncertainty is taken conservatively at half the dead time at \ns{2.5}. The systematic error is calculated in the same way as for the pileup amplitude uncertainty,
    \begin{align}
        \delta R = \sigma_{P_{t}} \times \frac{dR}{dP_{t}},
    \end{align}
where $P_{m}$ is the value of the pileup time-shift. The sensitivities of $R$ to the pileup time-shift and corresponding systematic errors for the Run~1 precession frequency analysis datasets are given in \tabref{tab:systematicError_pileupTimeShift}.
...
\centering
    ...
    \begin{subfigure}[]{0.45\textwidth}
        ...
        \caption{Sensitivity of $R$ to value of pileup time-shift. Units are in ppm.}
    \end{subfigure}
...
The second part of the pileup phase error comes from the choice of constant $C$ in the calculation of the energy of the pileup doublet as given in \equref{eq:Edoublet}. If the energy of the pileup pulses are systematically miscalculated, then doublets will be added or lost near the energy threshold applied when constructing the pileup spectrum, leading to an error on the phase. In order to calculate the systematic error from this parameter the parameter $C$ is scanned over in steps of 0.01 from 0.9 to 1.1, \figref{fig:PEscan}. As in the pileup amplitude error, the systematic error on $R$ is calculated as 
    \begin{align}
        \delta R = \sigma_{C} \times \frac{dR}{dC},
    \end{align}
where $C$ is the value of the pileup energy scaling constant. Similarly to the pileup amplitude error, there is a clear minimum in the \chisq plot which can be used to estimate the uncertainty in the pileup energy scale. \tabref{tab:systematicError_pileupC} gives the sensitivities of $R$ to the pileup energy scale, the associated uncertainties, and the corresponding systematic errors. As shown the value for $C$ which produces the minimum in the \chisq plot is consistent with 1\footnote{Because the spatial separation is turned off in the clustering portion of the reconstruction, a value of $C = 1$ is to be expected.}. Of interest to note is that the sensitivity of $R$ to $C$ in the 60h dataset is noticably larger than in the rest of the datasets. The origin of this is currently unknown.
...
\centering
    ...
    \begin{subfigure}[]{0.45\textwidth}
        ...
        \caption{Sensitivity of $R$ to value of pileup energy scale. Units are in ppm.}
    \end{subfigure}
...
% \begin{tabular}{@{\extracolsep{\fill}}lcccK}
  ...
  \hline
\end{tabular*}
\caption[Systematic error due to fixed pileup energy scale factor]{Systematic error due to the fixed pileup scale parameter $C$ in the Ratio Method fits for the Run~1 precession frequency analysis. The bold column gives the systematic error on \R. Units for $dR/dC$ and $\delta R$ are in ppb.}
\label{tab:systematicError_pileupC}
\end{table}


\tabref{tab:PileupErrorsTotal} gives the quadrature sum for the total pileup systematic error for the Run~1 precession frequency analysis datasets. As shown for each dataset the total error is below the target error of \SI{40}{ppb} in spite of the contamination in the pileup shadow method. For future runs of the experiment with increased rate and therefor increased pileup, these errors may grow. In that case either the pileup shadow method needs to be improved to account for the contamination and pileup triplets, or a different pileup construction method needs to be used.
...
-need to describe scans a little more deliberately - scan over x parameter in steps of x units, also refer to the slope as the sensitivity of R to blah
