...
\begin{tabular*}{1\linewidth}{@{\extracolsep{\fill}}lcccccc}
  ...
  \hline
\end{tabular*}
\caption[Dataset CBO frequency model parameters]{Fixed parameters in the CBO frequency model \cite{CBOFreqTrackingElog,JamesPersonalComm}. \textbf{Here I source personal communication with James, as I don't think there's an actual presentation with these numbers anywhere. Plus these numbers might change if certain runs and things are dropped out.}}
...
\textbf{include latest VW work, some plots, etc.}
...
In the fitting of the ratio...


\begin{figure}[]
    \centering
    \includegraphics[width=\textwidth]{VWRatioDiff}
    \caption[Difference in ratio value with and without the VW effect as a function of VW frequency]{Plotted is the difference in the maximum value of the ratio with and without a VW function included, as a function of both the amplitude of the VW effect and the VW frequency in units of the \gmtwo frequency, in a toy Monte-Carlo simulation. Note that this is not $\boldsymbol{R}$ the frequency fit parameter, but the actual value of the ratio fit function. The difference reaches a minimum for even multiples of the \gmtwo frequency, where the VW effect divides out almost entirely. The $n = 0.108$ datasets, including the 60h and Endgame datasets, live at the bottom center of this plot, marked by a red circle, where the VW frequency is nearly equal to 10 times the \gmtwo frequency, and the difference in the ratio is approximately \SI{5e-6}{} at 30 $\mu s$. Plot courtesy of James Mott. \textbf{update picture as necessary}}
...
parameters typically fixed in ratio method...
include James plot
include my plot I think showing essentially the same thing and describe it briefly
might want to just include an early time FFT with no caption or anything just temporarily in case I end up putting that in
