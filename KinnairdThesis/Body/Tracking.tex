...
After the track fitting, the returned parameters in $UV$ space can be turned back into the tracker reference frame coordinates. Plots for the fitted vertical and horizontal momenta, positions, and corresponding residuals are shown in Figures~\ref{fig:MaterialFittedMomentaComponents} and \ref{fig:MaterialFittedPositionComponents}. The forward momenta plots are omitted since the majority of the total momenta is in the forward direction. More extensive plots than what is shown here can be found in \refref{trackfittingdoc}.
    ...
    \begin{figure}[]
      \centering
      \includegraphics[width=.5\textwidth]{MaterialTruthLR_EnergyLost}
        \caption[Simulated true energy loss vs Geane predicted energy loss for tracks]{Simulated true energy loss (black) vs Geane predicted energy loss (red) between the first and last hits of fitted tracks. There is a difference between the two curves coming from the difference in physics lists between the main Geant4 simulation and the Geane prediction. The energy loss is in general very small compared to the magnitude of the momentum due to the low amount of material in the tracker, so this difference is acceptable. Sources of energy loss come from ionization and bremsstrahlung processes, which account for the long Landau tail running off to infinity. Originally the Geane physics list was taking too much energy away due to bremsstrahlung processes, so the energy loss calculations were modified \cite{trackfittingdoc}.}
    ...
    \centering
        ...
        \begin{subfigure}[t]{0.45\textwidth}
            ...
            \caption{$V$}
        \end{subfigure}% 
    \caption[Track parameter truth pulls for fitted tracks in the \gmtwo Geant4 simulation with material effects included]{Truth pulls for the five fitted track parameters at the starting plane of the track for fitted tracks in the full \gmtwo Geant4 simulation with material effects included. The plots are shown on a log scale, and each are fit to a Gaussian in red. Each fit is close to a unit Gaussian with a mean of zero and an RMS of 1, but there are tracks which due to the material effects lie outside the Gaussian.}
    \label{fig:MaterialTruthPulls}
    \end{figure}

    \begin{figure}[]
      \centering
      \includegraphics[width=.4\textwidth]{MaterialTruthLR_NumberIterations}
        \caption[Number of iterations for the track fitting to converge]{Number of iterations for the track fitting to converge. The track fitting does not take less than three iterations, and the number of iterations is capped at ten.}
    ...
    \centering
        \begin{subfigure}[t]{0.45\textwidth}
            ...
            \caption{The number of planes hit peaks at 10, and falls off relatively linearly to the maximum number of planes at 32. There is a track finding cut of 6 that shows up in this plot as well.}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}[t]{0.45\textwidth}
            \centering
    ...
    \centering
        ...
        \begin{subfigure}[t]{0.45\textwidth}
            ...
            \caption{The absolute momentum residual between the reconstructed and true energy loss. The RMS is approximately $\SI{40}{\MeV}.$}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}[t]{0.45\textwidth}
            \centering
            \includegraphics[width=\textwidth]{MaterialTruthLR_Fitted_PTotal_RelResidual}
            \caption{The relative momentum residual between the reconstructed and true energy loss. The RMS is approximately $\SI{2.4}{\%}$. This plot includes tracks of all momenta, in general the resolution of the track fitting estimate of the momentum is around 2\%.}
        \end{subfigure}%
    ...
    \centering
        \begin{subfigure}[t]{0.45\textwidth}
            ...
            \caption{Vertical momentum distribution. This distribution is bounded by the storage region and muon storing which limits the vertical momentum of the muons and by extension the decay positrons.}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}[t]{0.45\textwidth}
            \centering
            \includegraphics[width=\textwidth]{MaterialTruthLR_Fitted_Px}
            \caption{Horizontal momentum distribution. This distribution has an interesting shape due to the acceptance of the tracker. It is perhaps more informative to look at the radial momentum distribution, which is shown later in \figref{blach}.}
        \end{subfigure}
        \vspace{2mm}
        \begin{subfigure}
        [t]{0.45\textwidth}
            ...
            \caption{The vertical momentum reconstruction has a width of about $\SI{4}{\MeV}$.}
        \end{subfigure}%
        \hspace{5mm}
        \begin{subfigure}[t]{0.45\textwidth}
            \centering
            \includegraphics[width=\textwidth]{MaterialTruthLR_Fitted_Px_AbsResidual}
            \caption{The horizontal momentum reconstruction has a width of about $\SI{3}{\MeV}$.}
        \end{subfigure}%
    \caption[Fitted track vertical and horizontal momenta and corresponding residuals to truth]{Shown are the fitted vertical and horizontal momentum components (top) for tracks at the entrance to the tracker. On the bottom are the absolute residuals between the reconstructed momenta and truth.}
    ...
    \centering
        \begin{subfigure}[t]{0.45\textwidth}
            ...
            \caption{Vertical position distribution. This distribution is bounded by the storage region and muon storing which limits the vertical position of the muons and by extension the decay positrons.}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}[t]{0.45\textwidth}
            \centering
            \includegraphics[width=\textwidth]{MaterialTruthLR_Fitted_X}
            \caption{Horizontal position distribution. This distribution has an interesting shape due to the acceptance of the tracker. It is perhaps more informative to look at the radial position distribution, which is shown later in \figref{blach}.}
        \end{subfigure}
        \vspace{2mm}
        \begin{subfigure}
        [t]{0.45\textwidth}
            ...
            \caption{The vertical position reconstruction has a width of about \mum{740} at the entrance of the tracker, which is expected just from geometry arguments from the angles of the straws which measure mostly horizontally.}
        \end{subfigure}%
        \hspace{5mm}
        \begin{subfigure}[t]{0.45\textwidth}
            \centering
            \includegraphics[width=\textwidth]{MaterialTruthLR_Fitted_X_AbsResidual}
            \caption{The horizontal position reconstruction has a width of about \mum{140} at the entrance of the tracker, which is expected since the straws measure mostly horizontally with a straw resolution of \mum{150}.}
        \end{subfigure}%
    \caption[Fitted track vertical and horizontal positions and corresponding residuals to truth]{Shown are the fitted vertical and horizontal position components (top) for tracks at the entrance to the tracker. On the bottom are the absolute residuals between the reconstructed positions and truth.}
...
%differences in mc to data results are not due to bad track fitting but instead a mismatched simulation
