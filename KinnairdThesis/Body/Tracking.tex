...
Before actual data can be fitted, the left-right ambiguity problem needs to be dealt with. Since straws only measure drift circles and not actual U or V positions, and since tracks are in general passing forwards through the detector, there is an ambiguity as to which side of the wire a particle passed for each hit. In general if even a single left-right choice for a hit is wrong then a track will fit poorly. There are a couple of different track fitting modes used to deal with this problem, which are detailed in \refref{trackfittingdoc} and summarized in \refref{leftrightpresentation}.

\subsection*{wireFit}

The first fit to any set of tracks in data is to do a fit to the wire centers, which requires no left-right information. The errors on the measured hits are set as the RMS of a uniform distribution with a width of a straw diameter. After a wire fit, an approximation of the best fit track is made which provides some left-right information. The number of fitting iterations is capped at three for the wire fit.

\subsection*{mainfit}

The primary fit mode is called ``mainFit,'' and it is the mode used when analyzing the data. After the wire fit is completed, the predicted track parameters at each measurement plane are compared with the locations of the straw wires that were hit. The left-right choices for each hit are set depending on which side of the wire the predicted track from the wire fit went. The fitting is repeated with the new measurement positions and corresponding DCA errors, and at each iteration of the fitting the left-right choices for each hit are updated based on where the previous predicted track parameters ended up. This is the fastest fit mode and with this method about 66\% of tracks are fitted well.

\subsection*{fullSeqFit}

The seconday fit mode is called ``fullSeqFit.'' This fit mode does a better job of determining the left-right choices for the hits in a track, but is much slower than ``mainFit.'' Since there is no statistics problem when it comes to tracks, this fit mode is usually ignored. Here is given a short summary, see \refref{trackfittingdoc} for more technical details on all parts of this fitting mode.

After a wire fit, this fit mode first uses the geometry of the straw layers to resolve some of the left-right choices for a hit on a track. The left-right ambiguity is partially resolved through the shift in layers for each view as detailed in \secref{sec:StrawTrackers}, where if a particle passes through both layers in a view in a certain way it can be seen to go to the left of one wire and the right of the other. This means that for a certain set of tracks with a certain angle at some planes, the left-right choices for a set of hits are known precisely \cite{JoeLR}. For these hit planes the left-right choices are locked in. 

For the remaining hits where the left-right choices are unknown, an approximate \chisq is calculated for each combination of left-right hits, $2^{N_{\text{unknown}}}$, using the Geane track objects calculated by the wire fit but with the measured parameters set to the different left-right combinations. This is sped up by only checking the left-right combinations for the U or V layers at a time respectively, $2^{N_{\text{U,unknown}}} + 2^{N_{\text{V,unknown}}} \ll 2^{N_{\text{unknown}}}$. This allows for a measure of how good a left-right combination is to be determined without having to propagate every track in Geane in the full fitting. Once the approximate \chisq's have been calculated, the best tracks are fitted with the full Geane propagation and \chisq minimization algorithm. Typically the best 10 to 15 combinations are fitted, and the final track with the best \chisq is taken as the track. This gets the left-right choices for the fitted track right 99\% of the time with simulated data, but is very slow due to the combination checking and even more so the Geane propagation for each of the best track combinations. If the track fitting can be sped up significantly or a majority of the left-right choices constrained upstream somehow, then this fit mode would be more useful. 
...
%differences in mc to data results are not due to bad track fitting but instead a mismatched simulation
    ...
    \centering
        ...
        \begin{subfigure}[t]{0.47\textwidth}
            ...
            \caption{}
        \end{subfigure}
    \caption[Fitted tracks in data compared to Monte-Carlo, track times and total momentum]{}
    ...
    \centering
        ...
        \begin{subfigure}[t]{0.47\textwidth}
            ...
            \caption{}
        \end{subfigure}%
    \caption[Fitted tracks in data compared to Monte-Carlo, radial and vertical momentum and position distributions]{}
...
-for showing plots include a bulleted list or table of the tracking cuts made - doc db 16282 and 16420
