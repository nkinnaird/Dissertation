...
Plugging in and dividing the common terms $(N_{0}e^{-t/\tau}/4)$,
	\begin{align}
		R(t) = \frac{\splitfrac{2(1 + A \cos(\omega_{a}t)) - e^{-T/ 2\tau} (1 + A \cos(\omega_{a}t + \omega_{a}T/2))}{ - e^{T/ 2\tau} (1 + A \cos(\omega_{a}t - \omega_{a}T/2))}} {\splitfrac{2(1 + A \cos(\omega_{a}t)) + e^{-T/ 2\tau} (1 + A \cos(\omega_{a}t + \omega_{a}T/2))}{ + e^{T/ 2\tau} (1 + A \cos(\omega_{a}t - \omega_{a}T/2))}}.
	\end{align}
...
where
	\begin{align}
		C = \frac{1}{16} \Big(\frac{T}{\tau}\Big)^{2} \approx 2.87 \times 10^{-4}.
	\end{align}
...
and since $C$ is small, the denominator can be manipulated such that
	...
	\end{aligned}
	\end{equation}
after dropping terms of $\mathcal{O}(C^{2})$ and higher. In practice the last term is omitted since it has a minimal effect on the fitted value of $\omega_{a}$~\cite{LDThesis}, and one arrives at
	\begin{align}
...
so that
	...
	\end{aligned}
	\end{equation}
(These factors out front are close to 1/4 since $e^{\pm T/ 2\tau} \approx e^{\pm 4.35/ 2*64.4} \approx 1.034, .967$.) Then instead $R(t)$ becomes 
	\begin{align}
...
and $C(t)$ can encode any other effects in the data that need to be fitted for, such as the CBO,
	...
	\end{align}
Additionally, any other fit parameters such as $A$ or $\phi$ can be made a function of $t$. Using the non-approximated form for the final fit function gives greater confidence in the fit results for the high precision $\omega_{a}$ extraction necessary for the experimental measurement.


\section{Ratio errors}

 In order to determine the errors on the points in the formed ratio, Equation \ref{eq:ratio}, standard error propagation is used:
 	\begin{align}
...
This works because $V(t)$ and $U(t)$ are statistically independent datasets. Using standard error propagation again, 
	...
	\end{aligned}
	\end{equation}
Calculating out and simplifying the partial derivatives, (dropping the time-dependencies),
	\begin{equation}	
	\begin{aligned}
	...
	\end{aligned}
	\end{equation}
Combining and simplifying, the error formula is given as:
 	\begin{align}
