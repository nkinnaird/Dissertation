...
\begin{tabular*}{.8\linewidth}{@{\extracolsep{\fill}}lc}
  ...
  \hline\hline
    Parameter & Value \\
  \hline
    Energy threshold & $\SI{1700}{\MeV}$ \\
    Bin width & $\SI{149.2}{\micro s}$ \\
...
First an energy threshold needs to be chosen. As described at the end of \secref{section:WaIntro}, the optimal energy threshold is where the quantity $NA^{2}$ reaches the maximum, at least in the case of a five parameter fit. By scanning over the choice of energy threshold and fitting the resulting time spectra with \equref{eq:5parfunc}, the optimal energy threshold can be determined as seen in \figref{fig:OptimalEnergyThreshold} \footnote{Using the final fit function used, a slightly better estimate can be found. }. The optimal choice of energy threshold was determined to be $\SI{1700}{\MeV}$, in accordance with the cluster reconstruction energy calibration.
