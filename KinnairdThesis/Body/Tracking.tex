...
-minimization done in coordinate system given by ... 



The method for fitting and improving the track uses a global \chisq minimization algorithm that uses the transport and error matrices as described previously \cite{geanemanual,trajfit}. This straightforward global fitting algorithm works because of the minimal amount of material contained within the tracker. For denser detectors, other fitting algorithms such as a Kalman filter should be used \cite{Lavezzi}.


The $\chi^{2}$ for a track is defined as the residuals of predicted and measured parameters divided by their errors, summed over all hit planes:
