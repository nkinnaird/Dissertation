...
As referenced in \equref{eq:torque}, particles in a magnetic field will experience a torque which attempts to line up the magnetic dipole momement of the particle with the external field. Because of this, in a dipole field a particles spin will turn at the Larmour precesssion frequency
        \begin{align} \label{eq:ws}
            \vec{\omega}_{s} = -g\frac{q}{2m}\vec{B} - (1-\gamma)\frac{q}{\gamma m}\vec{B},
        \end{align}
where as before $m$ is the particles mass, $q = \pm e$ where $e$ is the elementary charge, \g is the g-factor, $\gamma$ is the Lorentz relativistic factor, and $B$ is an external magnetic field. The second term is a relativistic correction to the precession frequency called Thomas precession \cite{Jackson}. Similarly, particles with some momentum will orbit at the cyclotron frequency
...
By taking the difference between these two frequencies we arrive at the "spin difference frequency"
        ...
        \end{align}
a measureable frequency that is directly proportional to the anomalous magnetic moment $a$. Briefly note that if $g = 2$ as in a Dirac theory, then the particles spin would turn at the same rate as the momentum vector, and this spin difference frequency \wa would be identically 0. If this spin difference frequency for a muon and the external magnetic dipole can be measured, then then the anomalous magnetic moment of the muon \amu can be measured. The details of the former will be explained in this chapter, while that of the latter will be explained in \chapref{chapter:MagneticFieldMeasurement}.
...
\centering
    ...
    \hspace{10mm}
    \subcaptionbox{$\mu^{+}$ decay through a $W^{+}$ boson to a positron, muon anti-neutrino, and an electron neutrino.}
    {
    \centering
        \begin{tikzpicture}[baseline=(o.base)]
        \begin{feynhand}
...
\caption[Feynman diagrams for muon and pion decay]{Feynman diagrams for pion (left) and muon (right) decay.}    
...
\begin{figure}[]
    ...
    \caption[Muon decay pictures for maximum and minimum energy positrons]{improve this captiontest.: The single available decay state for maximum energy decay positrons. Due to the conservation of angular momentum and the single possible helicity states of the decay neutrino and anti-neutrino, the spin of the decay positron is exactly equal to the spin of the muon at the time of the decay.}
