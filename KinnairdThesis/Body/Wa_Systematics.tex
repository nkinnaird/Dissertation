...
Since corrections affect the final fitted wa value as in secref blah, any errors in the calculations themselves can be thought of as errors in the precession frequency mesaurment...


-explain where these errors come from - point back to equations for the correction
    - trackers for pitch correction which cares about vertical width
        - cite James' elba talk for pitch
    - fast rotation analysis for E field correction which cares about radial equilibrium
        - cite antoines actual notes on the separate datasets
-explain how other working groups are on it
    -mention how simulation efforts are under way as well for comparison
-quote preliminary estimations
-might want to talk a little bit about the bad resistors and how that is folded in to the above errors
    -cite dave rubins elba talk and one of his more recent updates
-remind of old e821 numbers - values and errors (maybe values in the conclusions and just the errors here)
...
% 60h: expected: ~160 ± 15 ppb
% That comes from 9.1 + 16 ppb from acceptance + 10 ppb from simulation/model uncertainty 


\cite{PitchReview,PitchElba}



Antoine numbers:

% 60h: CE = −463 ± 1 (stat) ± 27 (syst) ppb.
% 9d: CE = −463 ± 1 (stat) ± 36 (syst) ppb
% Endgame: CE = −467 ± 1 (stat) ± 20 (syst) ppb

\cite{AntoineEField60h,AntoineEField9d,AntoineEFieldEndgame}
...
\cite{DaveRubinElbaBadResistors,DaveRubinBadResistorsUpdate}
...
\begin{tabular*}{\linewidth}{@{\extracolsep{\fill}}lGGGG}
  ...
  \hline
\end{tabular*}
\caption[Systematic errors evaluated in the Run~1 precession frequency datasets]{Final systematic errors evaluted in the Run~1 precession frequency analysis to the 60h, HighKick, 9d, and Endgame datasets. All units are in ppb. The table is split into two sections. The upper section consists of systematic errors directly evaluated by the author while the lower section consists of preliminary systematic estimates by other working groups.}
