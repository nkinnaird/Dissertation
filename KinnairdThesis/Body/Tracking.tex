...
All stages of the track reconstruction have ultimately lead to the goal of determining muon beam parameters and dynamics. As described in \secref{sec:muonbeamdynamics}, the muon beam is not a fixed entity. How the muons move in the storage region ultimately affect the calculation of the E-field and pitch corrections, the weighting of the muon distribution with respect to the measured field, and the calorimeter precession frequency analysis. Here are presented plots which describe the muons in the storage region, for those muons which decayed to positrons which then passed through the tracking detectors. There is still on-going work in regards to analyzing the acceptance differences between the tracker and calorimeter, in order to make sure that the correct muon beam dynamics are described in regards to those positrons which hit the calorimeters and are responsible for the measurement of \wa. 
...
\end{itemize}
These cuts remove those tracks which fail or are too poor, and cut out those tracks which end up in the tails of the various distributions. By applying these cuts a clean sample of the extrapolated muon decay positions is extracted without biasing the results. 


First, a radial slice of the extrapolated beam distribution is shown in \figref{fig:BeamCrossSection}. As shown the beam is off-center due to kicker effects, but note that it is relatively well localized. The radial and vertical projections of this cross-section are shown in \figref{fig:RadialVerticalProjections}. The vertical distribution is very similar to that from the track fitting results, while the radial distribution of the stored muons has an interesting shape due to how the muons are stored. These two distributions in general appear wider than they actually are, because all tracks of all times are included in these plots. \figref{fig:RadialVerticalVsTime} shows the radial and vertical distributions of the muon beam as a function of time, where an oscillation in the distributions is immediately apparent. Plotted in \figref{fig:RadialVerticalMeans} are the means of the radial and vertical distributions respectively. This oscillation as described before is the CBO. It can be seen that over a course of a fill, the CBO starts to decay away, and there is a corresponding beam motion as a consequence.


\textbf{-a table of muon beam width and means might be useful...}


\begin{figure}[]
  \centering
  \includegraphics[width=0.7\textwidth]{BeamCrossSection}
    \caption[Extrapolated muon beam distribution cross-section]{Shown is a radial slice of the extrapolated muon distribution or beam spot. The beam is localized off center of the storage ring due to the kicker settings used, \secref{sub:kicker}. Plot courtesy of James Mott.}
...
\end{figure}
    ...
    \centering
        \begin{subfigure}[t]{0.47\textwidth}
            ...
            \caption{The radial position of $\SI{0}{mm}$ corresponds to the magic radius of $\SI{7.112}{m}$. The radial distribution is peaked almost $\SI{2}{cm}$ radial outward from the center.}
        \end{subfigure}
        \hspace{5mm}
        \begin{subfigure}[t]{0.47\textwidth}
            \centering
    ...
    \centering
        ...
        \begin{subfigure}[t]{0.47\textwidth}
            ...
            \caption{The vertical distribution of the muons is relatively stable as a function of time, but there is an oscillation that can just barely be seen, corresponding to the CBO.}
        \end{subfigure}
    ...
    \centering
        ...
        \begin{subfigure}[t]{0.47\textwidth}
            ...
            \caption{The vertical distribution mean oscillates at the vertical waist frequency, as described in \tabref{tab:frequencies}. As shown for these early track times the beam is actually stored slightly higher than the center of the storage region.}
        \end{subfigure}
...
As described in \secref{sec:Run1}, the CBO characteristics of the beam were found to be changing over the course of a fill. Using the tracking results shown here, the CBO frequency and amplitude can be characterized \cite{cbofrequency}. The CBO frequency as a function of time in fill is shown in \figref{fig:CBOFrequency}. The CBO amplitude as a function of time in fill is shown in \figref{fig:CBOAmplitude}. Both of these characteristics will be folded into the precession frequency analysis as will be described later.
