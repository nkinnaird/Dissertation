...
% To date, direct searches for new physics at the LHC have thus far not found anything that differs from the Standard Model.  The other way to search for new physics is to make a very precise measurement of a quantity that is predicted by the Standard Model such as a_μ in order to search for a deviation, or to measure something forbidden by the SM such as muon to electron conversion.
...
\centering
	...
	\end{subfigure}
\caption[QED diagrams contributing to the magnetic moment]{The first of many QED diagrams contributing to $a$. Feynman diagrams made using TikZ-FeynHand~\cite{tikz-feynman,tikz-feynhand}.}	
...
% propagators from both the leptons and EW gauge bosons will be included. Per usual Feynman rules, the propagators will contain the masses of the interacting leptons and bosons. 
%For this reason knowing these contributions only up to two-loop level is sufficient, since higher order diagrams will be suppressed even further. 
...
\centering
	...
	% \end{subfigure}
\caption[Electroweak diagrams contributing to the magnetic moment]{First order weak diagrams contributing to $a$. Feynman diagrams made using TikZ-FeynHand~\cite{tikz-feynman,tikz-feynhand}.}	
...
\centering
	...
	\centering
		...
		\end{feynhand}
		\end{tikzpicture}
	\caption{The first order HVP Feynman diagram.} 
	\label{fig:HVP1}
	\end{subfigure}
	\hspace{3mm}
	\begin{subfigure}[t]{0.4\textwidth}
...
% including KLOE, CLEO, BaBar, and BESIII~\cite{KLOE,CLEO,BaBar,BESIII}. Further cross-section data are currently being acquired and analyzed from those experiments as well as the SND and CMD-3 detectors at the VEPP-2000 collider in Novosibirsk, Russia~\cite{SND,CMD3}.
...
% If the calculation is supplemented with the cross-section data described above, then this method provides the most precise determination of $a_{\mu}^{\text{HVP;LO}}$ as shown in \figref{fig:HVPComparison}. 

% \begin{figure}
%     \centering
%     \includegraphics[width=0.7\textwidth]{HVPComparison}
...
The second of these hadronic contribution parts is a higher-order, four-photon interaction, termed hadronic light-by-light (HLbL). Diagrams are shown in \figref{fig:HLbL}. The calculation of these diagrams has historically been model dependent, and has therefore been the most contentious part of the SM calculation. The value of the HLbL contributions to \amu from model estimates by the so-called `Glasgow consensus' is $a_{\mu}^{\text{HLbL}} = (10.5 \pm 2.6) \times 10^{-10}$~\cite{Prades:2009tw}.

% and adjusted due to axial exchanges is given by Nyffeler as $a_{\mu}^{\text{HLbL}} = (9.8 \pm 2.6) \times 10^{-10}$~\cite{Prades:2009tw,Nyffeler:2016gnb}.
...
\centering
	...
	\centering
		...
		\end{feynhand}
		\end{tikzpicture}
	% \caption{The most general HLbL diagram, where three photons are exchanged with a virtual hadrons bubble.}
	\end{subfigure}
	\hspace{3mm}
	\begin{subfigure}[t]{0.4\textwidth}
	\centering
	...
	\end{subfigure}
\caption[Hadronic light-by-light diagrams contributing to the magnetic moment]{HLbL diagrams contributing to \amu, where three photons are exchanged with a virtual hadrons bubble. The diagram on the right is technically contained within the diagram on the left, however it is displayed separately here in order to help illustrate the complexity of this contribution. Feynman diagrams made using TikZ-FeynHand~\cite{tikz-feynman,tikz-feynhand}.}	
\label{fig:HLbL}
\end{figure}
...
% A cumulative report on recent progress is given by N. Asmussen et al.~\cite{HLbL1}. 
...
% There is extensive on-going work in both the dispersive and lattice approaches for the hadronic contributions by the Muon \gmtwo Theory Initiative, comprised of many working groups from various institutions, with future consensus values expected sometime in the first half of 2019, and extensive plans for improvements going forward~\cite{TheoryInitiative}.

\begin{figure}
	\centering
	\includegraphics[width=0.8\textwidth]{amuSMcompare_KNT2019}
...
% In general, these \gmtwo experiments belong to a class of experiments which seek to measure a quantity predicted by the SM to high precision in order to look for a deviation which provides evidence of either new physics or shortcomings in the SM. 
...
% while scalar axion-like particles have mostly been ruled out


Future data taken from the LHC and other experiments will further constrain the possible models based on other experimental observables. The Fermilab E989 experiment was undertaken in order to improve the experimental precision of the \amu measurement, while verifying the previous result. Although, in general, there has been a lack of new physics results from other experiments, E989 has the potential to provide an indirect confirmation of new physics. Accordingly, interest in the E821 experiment and it's successor E989 has grown over time. The number of citations for the E821 results has been consistently high for 20 years as shown in \figref{fig:E821Citations}.
...
The E989 experiment has the goal of measuring \amu to \SI{140}{ppb} by 2021. This would be a factor of four improvement over the E821 result stemming from a twenty times increase in statistics, which was the dominant error in the previous experiment. Assuming the same central value for \amu is obtained, this new measurement would push the statistical significance of the discrepancy to approximately seven standard deviations, as shown in \figref{fig:AlexKPaperComparison}. The data comprising Run~1, gathered between April and July of 2018, is the subject of this thesis, and corresponds to a statistical uncertainty comparable to the E821 result. The Run~1 result by itself may provide strong evidence for new physics, depending on the new measured value for \amu.


% The possibility is that we will measure something new and exciting, either in Run 1 or the following runs. However, even if we do not, it is valuable in itself to resolve this theoretical and experimental conflict.



% from Aaron's thesis:
%~\cite{Czarnecki_2001} % Aaron's citation for 2001 possible explanations of g-2 - mostly supersymmetry which has been ruled out, but I should probably skim through the document - radiative mass mechanisms which I haven't talked about
%~\cite{Hagiwara_2018} % MSSM parameter spaces not fully excluded
%~\cite{Chen:2015vqy} % light dark higgs scalar
%~\cite{Ablikim:2017aab}% dark photon - simplest model excluded as far as I can tell
%~\cite{Marciano_2016} % axion-like particles

% from BSM presentation:
% % skipping first citation on MSSM, since it's from 2006
%~\cite{Leskow_2017} % leptoquarks, skipping the second leptoquark citation since it doesn't make any direct mention of g-2
%~\cite{crivellin2019bto} % 2 Higgs doublet models
%~\cite{Abe_2019} % scalar doublets extensions (not Higgs)

%~\cite{PhysRevD.98.113002} % other article which is a summary of others, also source for vectorlike fermions
...
% 	\end{feynhand}
% 	\end{tikzpicture}
% \caption[General diagram for new physics]{A general Feynman diagram, where the leptons couple to an external magnetic field B through some BSM physics. Feynman diagrams made using TikZ-FeynHand~\cite{tikz-feynman,tikz-feynhand}.}	
% \label{fig:BSM}
% \end{figure}
