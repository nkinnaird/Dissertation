...
where $d\langle\phi\rangle/dt$ is the change in average phase over the course of a fill. With some assumpations made with regards to stored and lost muon populations this can be written as \cite{MikeLosses}
    ...
    \end{align}
where $f_{s}$ is the fraction of stored muons to all injected muons, $f_{l}$ is the fractional loss rate, and $\Delta\phi_{s-l}$ is the phase difference between the stored and lost muon populations. The simulation studies in the afore-referenced document gives estimates of $f_{s} \approx 0.9$ and $\Delta\phi_{s-l} \approx \SI{1}{mrad}$, which are reasonable approximations of the scales of the real numbers. The fractional losses (the integral in \equref{eq:lambdalosses} times the final fitted $\kappa_{loss}$ parameter) for the Run~1 precession frequency analysis datasets as determined in this analysis are shown in \figref{fig:fractionallosses}. As shown the losses rise sharply at early times before leveling off to a constant rate around \mus{100} into the fill. Taking the first \mus{70} of the fit and assuming the loss rate is constant over the course of the fill, the approximate loss rates for the different datasets are given in \tabref{tab:systematicError_lostMuonBias}\footnote{While the loss rate levels off after \mus{100}, most losses occur mostly at early times, so this is a fine approximation.}. Applying the approximate loss probabilities into \equref{eq:phaseChangeLostMuons}, the systematic shifts in the precession frequency are found. They are included in \tabref{tab:systematicError_lostMuonBias}.
